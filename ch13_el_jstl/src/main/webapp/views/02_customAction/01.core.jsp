...
<body>
	...
	* 데이터를 출력하고자 할 때 사용하는 태그
	...
	<c:out value="${ outTest }" /><br>	<!-- escapeXml생략시 기본값은 true로 태그를 해석하지 않음(=>문자열로 취급) -->
	<c:out value="${ outTest }" escapeXml="false" /> <!-- 따라서 문자열로 취급을 위해선 escapeXml을 false로  -->
	...
	*  (c:if test="조건식")
	  ...
	  - 조건식은 test 속성에 작성(단, EL구문으로 기술해야 됨)
	  - jsp로 조건문 작성시 <% %>로 끊었다 열었다 하며 작성하던걸 여기선 EL문으로 한번에 작성 가능
	</pre>
	...
	<c:if test="${ num1 >= num2 }">
		<b>num1이 num2보다 크거나 같다</b><!-- 조건이 거짓이면 출력되지 않음 -->
	</c:if>
	<br>
	...
	* (c:choose || c:when || c:otherwise)
	  ...
	  - 각 조건들을 c:choose의 하위요소로 c:when을 통해서 작성(else문의 역할은 c:otherwise가 수행)
	</pre>
	<!-- c:choose => if_else문을 시작한다는 태그 -->
	<c:choose >
		<c:when test="${ num1 gt 20 }">
			<b>num1은 20보다 크다</b>
		</c:when>
		<c:when test="${ num1 ge 10 }">
			<b>num1은 10보다 크거나 같다</b>
		</c:when>
		<c:otherwise>
			<b>num1은 10보다 작다</b>
		</c:otherwise>
	...
	<h3>4. 반복문</h3>
	<pre>
	* for loop문 - (c:forEach var="변수명" begin="초기값" end="끝값" [step="증가값"])<!-- step 증가값 기본값 1임 -->
	* 향상된 for문 - (c:forEach var="변수명" items="순차적으로 접근하고자 하는 객체(배열|컬렉션|등등)" [varStatus="현재 접근한 요소의 상태값을 보관하는 변수"])
	</pre>
	...
	<%
		...
		list.add(new Person("남고잉", 27, "남자"));
		list.add(new Person("킵고잉", 49, "남자"));
	%>
	
	<%-- ArrayList 객체를 jsp태그<% %>로 만들었기 때문에 ${}로 쓰면 인식못함 --%>
						...
						<c:otherwise>
							<c:forEach var="p" items="${plist}" varStatus="status">
								<tr>
									...
									<td>${status.count}</td> <!-- count속성 : 1부터 시작 -->
									<td>${p.name}</td>
									<td>${p.age}</td>
									<td>${p.gender}</td>
								</tr>
	...
	* (c:forTokens var="변수명" items="분리시키고자 하는 문자열" delims="구분자")
	  ...
	  - java의 spolit("구분자") 또는 StringTokenizer와 비슷한 기능
	</pre>
	...
	<pre>
	* url 경로를 생성하고, 쿼리스트링을 정의 할 수 있는 태그
