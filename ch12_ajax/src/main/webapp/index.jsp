...
<body>
	...
	<p>
		...
		서버로부터 데이터를 가져와 전체 페이지를 새로 고치지 않고 <b>일부만 로드</b>할 수 있게 하는 기법<BR>
	...
	* jQuery방식으로 AJAX통신
	...
	$.ajax({<!-- 여기있는 52~62행은 기본적으로 들어감. 단, type, data는 없으면 안써도 됨 -->
	...
	* 주요속성
	- url : 요청할 url(필수속성)
	- type|mothod : 요청전송방식(get/post)<!-- type, method는 같은 말임 둘 중 하나 아무거나 쓰면 됨-->
	- data : 요청시 전달할 값
...
<!-- <form name="idCheck" action="idCheck.me">
					...
					success : function(result) {							
						if(result == 'idN') {
							...
							if(confirm("아이디를 사용하시겠습니까?")) {
								$("form input[value=회원가입]").removeAttr("disabled");
	...
	<script type="text/javascript">
		$(() => {
			const $idInput = $("#id");
			$idInput.keyup(function() {/* .keyup()은 키보드를 한번 누를때 마다 호출됨 */
...
<!-- 
 ...
 -->	
				...
				$.ajax({
					...
					success : function(result) {
						...
						const value = "<li>ID : " + result.userId + "</li>"
									...
									+ "<li>GEN : " + result.userGen + "</li>"
									+ "<li>Email : " + result.userEmail + "</li>";
									$("#output5").html(value);
									*/
	...
	<button id="btn6">회원 전체조회</button><br>
